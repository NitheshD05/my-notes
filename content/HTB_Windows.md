# Hack The Box (HTB) Windows Writeups

Welcome, fellow hackers, to the Windows realm of my HTB write-ups! Explore the chronicles of my conquests over Windows-based HTB boxes. Each write-up is filled with detailed step-by-step instructions, expert tips, and the exciting stories of my victories. Grab your favourite hacking snacks and join the journey!

<img src="windows-writeup-img.jpeg" alt="HTB Linux Writeups" class="htb-Writeup-img">

<style>
  .htb-Writeup-img {
    width: 100%;
    max-width: 800px;
    border-radius: 8px;
    box-shadow: 0 0 10px rgba(0, 191, 255, 0.8); /* Initial glow effect */
    transition: box-shadow 0.3s ease-in-out, transform 0.3s ease-in-out; /* Smooth transition for glow and zoom effects */
  }

  .htb-Writeup-img:hover {
    box-shadow: 0 0 20px rgba(0, 191, 255, 1); /* Enhanced glow on hover */
    transform: scale(1.05); /* Zoom effect on hover */
  }
</style>

---

## Writeups

### [Arctic](Arctic.md)

### [Authority](Authority.md)

### [Bank Robber](BankRobber.md)

### [Bastard](Bastard.md)

### [Bastion](Bastion.md)

### [Blue](Blue.md)

### [Bounty](Bounty.md)

### [Chatterbox](Chatterbox.md)

### [Conceal](Conceal.md)

### [Devel](Devel.md)

### [Forest](Forest.md)

### [Grandpa](Grandpa.md)

### [Granny](Granny.md)

### [Jeeves](Jeeves.md)

### [Jerry](Jerry.md)

### [Legacy](Legacy.md)

### [Optimum](Optimum.md)

### [Secnotes](Secnotes.md)

### [Silo](Silo.md)

### [Support](Support.md)

---

Happy hacking and enjoy the write-ups!
